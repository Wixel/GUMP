...
class GUMP
{
    ...
    private function parse_rule($rule)
    {
        ...
        if (is_array($rule)) {
            return [
                'rule' => $rule[0],
                'param' => $this->parse_rule_param($rule[1] ?? [])
            ];
        }
     ...
     * @throws Exception
     */
    private function call_validator(string $rule, string $field, array $input, array $rule_param = [])
    {
     ...
     * @return array
     */
    private function generate_error_array(string $field, $value, string $rule, array $rule_param = [])
    {
        return [
            'field' => $field,
            'value' => $value,
